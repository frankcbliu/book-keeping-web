...
const Classification: React.FC<Props> = ({ ledgerId }) => {
    ...
    useEffect(() => {
        if (!ledgerId)
            return
        classificationApi.classificationList({ 'ledger_id': ledgerId }).then((res: any) => {
    ...
    useEffect(() => {
        if (!data || !data.cur_classification_id)
            return
        subClassificationApi.subClassificationList({ 'classification_id': data.cur_classification_id }).then((res: any) => {
            const { sub_classifications } = res.data
