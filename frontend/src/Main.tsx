...
import { LedgerItem } from './services/ledger-service/index.interface';
...
const Main: React.FC = () => {
    ...
    const menuItems = useMemo(() => (ledgerList.map(({ id, name }) => ({ key: String(id), label: name })).concat(...FixedMenuItems)), [ledgerList]);

    useEffect(() => {
        ledgerService.getLedgerList().then((ledgerList) => {
